...
Authrouter.post("/sync-user", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const existingUser = await config_1.default.user.findUnique({
                where: { id }
            });
            if (existingUser) {
                console.log('👤 User already exists in database');
            ...
            const newUser = await config_1.default.user.create({
                data: {
                    id,
                    email,
                    name: name || '',
...
Authrouter.patch("/update-user", async (req, res) => {
...
//     }
// });
Authrouter.patch("/verify-email", async (req, res) => {
    console.log(`👆👆👆👆👆 Route called: /verify-email`);
    try {
