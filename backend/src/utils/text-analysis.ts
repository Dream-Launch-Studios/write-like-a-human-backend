 ...
 */
const calculatePassiveVoicePercentage = (text: string): number => {
 ...
 */
const calculateFirstPersonPercentage = (text: string): number => {
    const words = text.toLowerCase().split(/\s+/).filter(word => word.length > 0);
    if (words.length === 0) return 0;
 ...
 */
const calculateThirdPersonPercentage = (text: string): number => {
    const words = text.toLowerCase().split(/\s+/).filter(word => word.length > 0);
    if (words.length === 0) return 0;
 ...
 */
const calculatePunctuationDensity = (text: string): number => {
 ...
 */
const calculateNGramUniqueness = (text: string, n: number = 3): number => {
    const words = text.toLowerCase().split(/\s+/).filter(word => word.length > 0);
    if (words.length < n) return 1; // If text is shorter than n, uniqueness is 100%
 ...
 */
const calculatePredictabilityScore = (text: string): number => {
 ...
 */
const calculateAcademicLanguageScore = (text: string): number => {
    const words = text.toLowerCase().split(/\s+/).filter(word => word.length > 0);
    if (words.length === 0) return 0;
 ...
 */
export const calculateTextMetrics = (text: string): TextMetricsData => {
    // Count words
    const words = text.toLowerCase().split(/\s+/).filter(word => word.length > 0);
